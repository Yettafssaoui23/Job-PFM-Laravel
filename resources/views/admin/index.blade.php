...
@section('content')
        ...
        <tbody>
            ...
            <tr>
              ...
              <td>
                @if($post->status=='0')
                   <a href="{{route('post.toggle',[$post->id])}}" class="badge badge-danger"> Désactivé</a>
                    @else
                   <a href="{{route('post.toggle',[$post->id])}}" class="badge badge-success"> Activé</a>
                @endif
            </td>
              <td>{{$post->created_at->diffForHumans()}}</td>
              <td>
                  <a href="{{route('post.edit',[$post->id])}}"><button class="btn btn-warning">Modifier</button></a>
...
<!-- Modal -->
        ...
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel">Supprimer Publication</h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
              <span aria-hidden="true">&times;</span>
            </button>
          </div>
          <div class="modal-body">
            Voulez-vous vraiment supprimer <b style="color: red;">{{$post->title}}</b> ?
          </div>
